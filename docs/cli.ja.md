...
領域名は `code` / `system`(bootloader)/ `option` / `eeprom` / `ram`。minichlink の `flash` / `bootloader` 名は別名として受ける。**`option` は family 依存**(CH32M030 = `0x1FFF_F300`)なので、`read --region option` と `write --at option` は**attach 後に DB の base で解決し直す**(`erase` は option 領域を拒否するので影響なし)。

**`flash` の書込経路は 2 本ある**: 既定は probe へ WCH の flash loader stub を載せて image を流す経路。**stub が存在しない family(V00x = family byte `0x4E`)では、検証済み FLASH-controller profile があれば §4.2.1 の直接 controller 経路で page 単位に書く**。page の埋まっていない部分は `0xff` を書く(消去直後の page の値なので stub 経路と同じ結果)。`--restore-unwritten` は従来どおり program 前に page を合成するので、どちらの経路でも意味が変わらない。**stub 経路より遅い**(word ごとに write + BUFLOAD + busy 待ちの DMI 往復が要る): 2.5 KiB の sketch で通し ~17 秒、62 KiB 全面で ~6 分。

**`erase --range` / `--region` の実装状況(2026-09-01)**: `--all`(chip 全消去)に加え、**page 単位の部分消去**を実装。WCH-Link stub の write 経路は部分書き込みを受け付けない(probe が reason 0x55 で拒否)ため、**FLASH controller(0x4002_2000)を DMI 経由で直接叩く**新経路を追加(`DebugModule::flash_page_erase` / `flash_program_page`。KEYR/MODEKEYR unlock → FTER/FTPG + STRT/PGSTART → STATR busy 待ち)。消去は page 粒度なので **start と length を page 境界に揃えることを必須**とし(fail-closed。ズレは Usage error + page サイズを提示)、隣接 page を巻き込まない。`--region code[+off[+len]]` は probe 報告の flash サイズから解決。program は family 別に 3 方式(PgStart=V20x/V30x、Buffered=V003/V00x/X035/L103、V103=標準 16bit halfword+commit。§4.2.1)。**対応 family: V20x/V30x・V003/CH641(64B)・V00x(256B)・X035/CH643・L103・V103(128B)**。実機検証: V307 で page1 だけ消去し page0(reset vector)/page2 無傷、V003 で 64B page、V103 で 128B page 消去(隣接無傷)、**L103 で 256B page を surgical に消去(先頭 firmware・中間 blank 域とも無傷、program/verify 往復 OK)**。**注意: 消去済みセルの read 値は family 差あり(RM の系統 B=V20x/V30x=`0xe339e339`、系統 A=X035/V003=`0xff`)**ので、erase 完了判定は read でなく controller の STATR(BUSY クリア + WPRERR 無し)で行う。この直接 FLASH controller 経路は今後 flash SW breakpoint(trigger 無し core)と option byte 書き込みの土台にもなる。
...
- **option 書込 `write-raw` / `reset` / `protect` 実装(2026-09-01)**: option-byte programming を `DebugModule::flash_program_option_bytes`(minichlink の option 経路から転記)で実装。手順: FLASH_KEYR + OBKEYR(`0x4002_2008`=STM32F1 系 OPTKEYR)+ MODEKEYR unlock → CTLR の OPTER+OPTWRE+STRT で **全 option byte 消去** → 8 halfword を OPTPG+OPTWRE+STRT で書込(value+complement をそのまま。complement は呼び出し側責任)。**RDPR(halfword0)を最初に書く**ことで消去〜再書込の read-protect 窓を最小化。反映は system reset 後。**verify は soft reset を挟んでから read-back する(2026-09-06 修正)**: V103 は option 領域を reset まで書込前の像で読み続けるため、直後に読むと**書けているのに `verify-mismatch`(exit 30)**を返していた(CH549 Link + V103 で実測)。`option get` と同じ 16 byte レイアウト。**`write-raw <hex>`**(expert、16 byte)は RDPR≠`0xA5` なら「読み出し保護 ON になる」警告して `--yes` 必須。**`reset`**(2026-09-06 修正)は「工場デフォルト」を**一律 `0xff` で作らない**: 現在値を読み、**DB が定義する USER bit だけ RM の復位値へ戻し、DB が知らない bit は現在値を保つ**。Data/WRP は `0xff` にクリア。理由は実測([measured/option-bytes-2026-09-06.md](data-requests/measured/option-bytes-2026-09-06.md)): 出荷値は V003=`0xf7`(`RST_MODE`=`10b`)・V20x=`0x3f`・V307=`0xbf`(`RAM_CODE_MOD` は個体差)で、**`0xff` を書くと NRST の機能や SRAM/flash 分割が変わる**。**`protect on|off`** は現在値を読んで RDPR のみ変更(on=`0xFF`、off=`0xA5`)、他 byte 保持。**`off` は全 flash mass erase を伴う**旨・**`on` は読めなくなる**旨を明示し `--yes`/確認必須。既に目的状態なら no-op。**OB base は DB 由来(2026-09-06)**: `option_bytes.csv`(依頼 0003 / R-30)の family 別 base を生成 DB 経由で引く。**共通番地ではない**(CH32M030 = `0x1FFF_F300`)ため決め打ちをやめ、**DB に無い family は fail-closed**(capability-unsupported。推測して別番地を読み書きしない)。読み(`option get`)・書き(`write-raw`/`reset`/`set`/`protect`)・`recover` の RDPR 判定と工場値書込のすべてがこの base を使う。なお DB は L103/M030/V006/V205/X035 の option 書込を fast page(FTPG)手順に分類しているが、**本実装の half-word(OPTPG)経路は L103 実機で動作確認済み**なので分類では gate せず、未検証の FTPG 分類 family でのみ警告を出す。**構造化 `set`(kv: rdp=/nrst=/split= 等)は family 固有 USER bit=DB(依頼 0003)依存のため後続**。実機検証(L103): `write-raw` に現在値を round-trip 書込 → 不変・RDPR=`0xA5` 維持・flash 無傷・走行復帰、`reset` も同様、fail-closed(bad length / RDP-danger without --yes / protect off 既off の no-op / protect on without --yes 拒否)。`protect on/off` の実トグルは破壊的(on=保護・off=全消去)のため実機未トグル(primitive は round-trip で検証済み)。programming 失敗時は option が消去済み=read-protect の可能性を error に明示し `recover` を案内。
...
- **attach は target のクロックを変える(2026-09-25)**: WCH-Link の AttachChip は、**CH32L103 / V20x / V30x では**クロックを系統ごとの決まった PLL 設定に組み直し、**元に戻さない**(L103 / V203 は線上で、V307 は接続後に残る値で確認。**V003 は RCC に触れない**。その他の family は未観測。protocol §7a)。`dmdata` / `dmseq` / `rtt` / `sdi` はどれも attach を通るので、観測中の firmware は **次の reset まで probe のクロックで走る**(UART の baud・タイマ・PWM がずれる。V20x / V30x で HSE + PREDIV / PLL2 を使う firmware は PLL の入力も変わる)。元の値は AttachChip の中で上書きされるので ch32rv は戻せない。monitor は、組み直しが分かっている family(L103 / V20x / V30x)でだけ開始時に warning `attach-reclocks-target` を出す。**firmware 自身のクロックのまま見たいときは attach のあとに reset する**: `ch32rv run <elf> --no-flash --source dmdata|dmseq|rtt`(接続 → DMI reset-halt → 流す。firmware は再起動する)か、ボードの reset。attach しないのは `uart` だけ。同じことは `target info` / `read` / `dbg` / `arduino monitor` にも当てはまる(警告は monitor だけが出す)。
...
| `dmseq` | `SerialDMSeq` | **host が DMI で DM data0/data1 を polling**(通番 + CRC-8 の framing。6 byte out / 2 byte in) | 任意(CH549 も可) | 双方向 | `dmdata` と同じ mailbox の**別 framing**。CDC を使わない。core は halt しない |
...
- **`dmseq` は `dmdata` と同じ DM data0/data1 を使う別 framing**(OEP `target.console` framing 2。仕様は oep-spec の `docs/target-console-dmseq.ja.md`、2026-09-24 合意)。両方向に 1 bit の通し番号と全 word の CRC-8 があり、**答えの書込みが落ちたときの重複と、それを直そうとしたときの欠落を区別できる**のが `dmdata` との違い。CRC は必須で、実際 attach が DATA0 に残す `0xffffffff` や CH32V30x の flash が残す `0xe339e339` を弾くのにも効く(後者は bit7 が 0 なので、CRC が無いと target が有効な答えとして受け取る)。target sketch は `SerialDMDATA` と `SerialDMSeq` のどちらか一方のみ(排他)。host 側の追加通知: target が答えを待つのをやめると warning `dmseq-target-timeout`(その間の出力は捨てられている)、3 秒フレームが来なければ warning `dmseq-no-console`。
- port は VID/PID/serial/interface から決め、COM 番号・`/dev/tty*` の番号に依存しない。`--probe name:bench-01` から同一物理 device の CDC を引けることが HIL の要件。
- 実装は 2 backend に割れる: **CDC serial backend**(`uart`/`sdi`。`sdi` は先に enable command)と **DMI backend**(`dmdata`/`dmseq`/`rtt`。core を halt せず running 中に DMI read/write)。
...
- **実装状況(2026-09-02)**: `db list` / `db info` を実装(cmd_db.rs、デバイス不要)。生成済み DB(`crates/target/generated/skus.csv` を `include_str!` で埋め込み)を列挙/表示。`db list --family <f>` は family/SKU prefix で絞り込み、`--verified-only` は実機確認済み(実測6台)のみ。`db info <SKU>` は family/device_id/flash/sram/verified を表示。JSON 可(arduino B-3=`db list --json` 対応)。DB は `cargo xtask db-gen` が ch32-device-data(pinned)から生成し commit する(§4.3 target info の chip_id→SKU 解決、§4.3 option get の family-aware USER decode もこの DB を使う)。geometry は flash/sram に加え **page/fast erase・fast program・block erase**(`flash_geometry.csv`、`db info` で表示)。**2026-09-06 追加**: `db info` は **消去済み word の読み出し値**(`erased:` 行。系統 A `0xffffffff` / B `0xe339e339`)と **FLASH controller の書込方式**(`flash prog:` 行。`buffered`/`direct` + commit bit + buffer load 幅 + DB の confidence)も出す。`flash_controller_profile`(§4.2.1 の直接 controller 経路)は手写しをやめて**この DB から導出**する。cli テストは、実機で駆動している 6 family について **DB 側に page 消去粒度・消去後の値・`conflict` でない書込手順が揃っている**ことを検査する(納品でどれかが欠けると `erase --range` / flash breakpoint が黙って無効化されるため)。方式・粒度が実機検証値と一致することは `ch32rv-flash` の単体テストで固定。option layout は register 系 CSV 取り込みで後続。
...
- **`capabilities` 実装(2026-09-02)**: cmd_capabilities.rs。attach して **probe variant(LinkE/CH549/…)+ FW + target family/SKU/配線(DB)** を読み、operation ごとに可否+理由を出す(human 表 + JSON)。判定(`build_matrix`、単体テストあり): `connect`(**1線 target × CH549 = 不可**。CH549 は 1線 SWIO 非対応、`Variant` + debug_wiring から)、`flash`(family stub 有無)、`erase --range / flash-bp`(FLASH-controller profile 有無。page サイズ+方式を理由に表示)、`gdb HW breakpoints`(attach 時に動的検出=V4C/V4F は 4)、`monitor sdi`(**LinkE 限定**)、`monitor dmdata`(任意 probe)、`recover power-off`(**LinkE/LinkW 限定**=target 電源制御)。**2 モード**: `--chip` 無し=attach して live target を判定、`--chip <SKU/family>` 有り=**probe info のみで静的判定(target 未接続でも可=pre-flight プランナ)**。static は probe を attach せず `probe_info()` で variant/FW を取り、`--chip` を family_byte(`family_byte_from_name`)+ series/配線(DB)へ解決。実機検証: L103(LinkE)全 yes、**V103(CH549 probe)は sdi/power-off が NO**、V003(LinkE,1線)connect yes、**静的 `--probe <CH549> --chip CH32V003` が target 未接続で `connect NO`(1線×CH549)を予測**、未知 `--chip` は usage error。各操作 command 側の統一 gating(exit 24)への配線は後続。
