...
- **attach は target のクロックを変える(2026-09-25)**: WCH-Link の AttachChip は、**CH32L103 / V20x / V30x / X035 では**クロックを系統ごとの決まった設定に組み直し、**元に戻さない**(L103 / V20x は PLL へ、X035 は AHB の分周を外して flash の wait を 2 に。L103 / V203 / X035 は線上で、V307 は接続後に残る値で確認。**V003 は RCC に触れない**。その他の family は未観測。protocol §7a)。`dmdata` / `dmseq` / `rtt` / `sdi` はどれも attach を通るので、観測中の firmware は **次の reset まで probe のクロックで走る**(UART の baud・タイマ・PWM がずれる。V20x / V30x で HSE + PREDIV / PLL2 を使う firmware は PLL の入力も変わる)。元の値は AttachChip の中で上書きされるので ch32rv は戻せない。monitor は、組み直しが分かっている family(L103 / V20x / V30x / X035、X035 と同じ controller の CH643 も含む)でだけ開始時に warning `attach-reclocks-target` を出す。**firmware 自身のクロックのまま見たいときは attach のあとに reset する**: `ch32rv run <elf> --no-flash --source dmdata|dmseq|rtt`(接続 → DMI reset-halt → 流す。firmware は再起動する)か、ボードの reset。attach しないのは `uart` だけ。同じことは `target info` / `read` / `dbg` / `arduino monitor` にも当てはまる(警告は monitor だけが出す)。
