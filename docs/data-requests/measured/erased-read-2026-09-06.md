...
| CH32L103C8T6 | 0x0E | `0xff` fill | A | 未書き込み領域(`0x0800F000`+32B)の直接読み。全 byte `0xff` |
...
- L103 は**未書き込み領域の直接読み**で確認(消去操作を伴わない = 非破壊。工場出荷時の消去状態がそのまま残っている領域)。
...
**L103 も直接測定済み**(2026-09-06 追測)。消去を伴わず、未書き込み領域を読むだけで確定できたので非破壊。→ 6 family すべてが直接読みの裏付けを持つ。
