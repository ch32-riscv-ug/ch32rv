...
- 状態: **納品受け入れ済(2026-09-02)。追加分も納品受け入れ済(2026-09-16)** — `CH32V006K8U6`(`0x00600600`)が `evidence/device_ids.csv` と `index/parts.csv` に収載された(ch32-device-data@e3e723a、F-76)。**データシートには載っておらず EVT が典拠**。実測した暫定行と全フィールド一致したので受け入れ、暫定 overlay を空にした。→ [measured/device-id-v006-2026-09-16.md](measured/device-id-v006-2026-09-16.md)。`evidence/device_ids.csv`(72行)+`device_id_addresses.csv` 納品。実測6台(V003/V103/V203/V307/L103/X035)と突き合わせ、rev bits [7:4] don't-care で全一致。`xtask db-gen` が `crates/target/generated/skus.csv` を生成、`target info` の chip_id→SKU 解決を実装(全5接続台で実機動作)。gap 7 series は未発売でデータ側も未収載(接続でき次第 measured 追記)
...
0. ~~**CH32V006 の package 違い**~~ — **完了(2026-09-16)**。`CH32V006K8U6` / `0x00600600` を収載済み。V006 の残り package(`0x0062` / `0x0063` 相当)は引き続き未収載([measured/device-id-v006-2026-09-16.md](measured/device-id-v006-2026-09-16.md))
