...
- **線上の実体**(verified 2026-09-25、LinkE fw 2.22): 2 線の L103 / V203 では **abstract memory access**(`command 0x0228xxxx`、post-increment)+ abstractauto による 585 clock / 15 語の burst(progbuf ではない)。1 線 SWIO の V003 では program buffer の routine を `command 0x00040000`(postexec のみ)で実行する方式。**読んだ後に ABSTRACTCS を一度も読まない**ので、アクセスが失敗しても USB 側には現れない。なお CH32L103 の未マップ番地(例 `0x60000000`)は silicon 自体が fault を出さずに 0 を返し、ch32rv 自前の progbuf 読みでも cmderr = 0 だった — 帯域内に失敗の合図がそもそも無い。ch32rv の `read` は、DB が知るメモリ領域の外を読むと warning `read-outside-memory-map` を出す。
...
- entry(`81 0f 01 01`)を送った probe は、**app が完全に無傷でも電源再投入で IAP mode のまま**再列挙する。BL が「IAP に留まれ」を不揮発に持っている。
- 書込を 63%(57,600 / 90,868 B)で SIGKILL し app を壊した状態でも、電源再投入後は同じく IAP mode。→ **中断がどの時点で起きても必ず再試行できる**(entry 不要で、IAP mode の device にそのまま書き直す)。
- 抜けるのは**終了 frame(`83 02 0000`)だけ**。単独で送ってよく、開始も書込も要らない: IAP mode の device に終了 frame を 1 つ送れば、すでに入っている app で起動する。
- つまり **IAP に入れたら片道**で、戻すには終了 frame か更新の完走が要る。host tool は「entry したら必ず終了 frame まで到達する」経路にし、脱出手段を別に持たせる。
- **壊れた app のまま終了 frame を送った場合は未検証**(probe を本当に失いうる唯一の操作なので試していない)。
...
|---|---|---|
| AttachChip `81 0d 01 02` | DMI 列(wch-protocols a5 の解析、6623b14): long 形式の DMSTATUS 問い合わせを 202 回(target は応答しない)→ WCH 独自 DM レジスタ `0x7e` / `0x7d` ← `0x5aa50400`(V003 では `0x7c` / `0x7d` も読む)→ haltreq ×2(**接続の間は halt**)→ `0x7f` を読む(= chip ID)→ CSR `0x7C0` ← `0x300` →(L103 / V203 / V307 / X035)**target のクロックを組み直し、元に戻さない** → ESIG `0x1FFFF7E0 / E8 / EC / F0` を abstract memory access(`data1 = 番地 → command 0x02200000 → data0`)で読む → 最後に resumereq。**クロックの組み直し**は CFGR0 の SW を HSI へ → PLL 停止 → 系統ごとの決まった PLL 設定で入れ直し → SW = PLL(その間の約 10 ms は HSI)。V20x は `RCC_CFGR2`(`0x4002102C`)を**読まずに 0** にし、L103(と X035)は FLASH_CTLR = `0x8080` / STATR = `0xB020` を書いたうえ、L103 は FLASH_ACTLR を最終的に**決め打ちの `0x1`** にする。**X035 は PLL を使わず**、CFGR0 を `0x00000050` から `0`(HPRE の分周を外す)にし、FLASH_ACTLR を `0 → 0x10 → 0x2` にする(FLASH_CTLR / STATR の書込も L103 と同じ。09-11 の X035 の線を新しい decoder で読み直した結果、3 回とも同じ)。接続後の CFGR0: V203 `0x0034040a`、L103 `0x001c040a`、V307 `0x0038040a`(reset 直後のスケッチ自身の値は V203 / V307 `0x0028000a`、L103 `0`。V307 は線でなく、target 自身に UART で報告させた試験 E162 で確認: PLLMUL が変わり APB1 が /2 になり、UART が化けてアプリ自身が新しい値を読んだ。L103 / V203 も同じ試験で UART の文字化けを確認、V003 は変化なし)。**V003 では RCC に触れない** | L103 / V20x / V30x / X035 では、reset を伴わない接続(`monitor`、`target info`、`read`、`dbg`)のあと、走っていたアプリは**次の reset まで LinkE のクロックで走る**(UART の baud・タイマ・PWM がずれる。HSE + PREDIV / PLL2 を使うアプリは PLL の入力も変わる — 推定、未実測)。元の値は AttachChip の中で上書きされるので、host が退避・復元することはできない。`flash --reset run` / `run` は最後の system reset でアプリが組み直すので無害。**attach 後に DATA0 に残る値は、最後に読んだ ESIG `0x1FFFF7F0` の語**(L103 は `0xffffffff`。V203 で見た `0xe339e339` も同じ番地と推定)— dmdata / dmseq の console が最初に読むゴミの出どころ |
| ChipInfo `81 11 01 05` | ESIG の読み出しは上の AttachChip の DMI 列の中にある。ChipInfo が返す flash 容量 / UID / chip ID はその値 | — |
