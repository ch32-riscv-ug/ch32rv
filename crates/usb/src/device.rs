...
/// cancels and drains the pending transfer so the host-side queue stays clean (same
/// discipline as probe-rs's nusb wrapper) - but the *device* may still be mid-stream, and
/// what it sends after the cancel surfaces as stale data in the next read on that endpoint.
/// Callers therefore size each request to finish well inside the timeout
/// (`WchLink::read_mem` in `ch32rv-wchlink` works in 16 KiB windows for exactly this reason)
/// rather than relying on the cancel to tidy up. The CH375 path has no timeout control
/// (the driver exposes none that is capture-verified) — transfers block, which is safe
/// under the request/reply pattern all callers use and rules out speculative reads there.
