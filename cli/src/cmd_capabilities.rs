...
pub fn capabilities(cli: &Cli) -> ExitCode {
    ...
    let variant = &session.probe_info.variant;
    let fw = format!(
        "{}.{:02}",
    ...
    let caps = build_matrix(variant, family_byte, wiring.as_ref());
    ...
    if cli.json {
        ...
        println!("probe:   {} (fw {fw})", variant.name());
        println!(
            "target:  {family}{}",
            sku.as_deref()
                .map(|s| format!(" / {s}"))
...
fn build_matrix(
    ...
    if one_wire && !is_linke {
        ...
        caps.push(Cap::yes("connect", "AttachChip succeeded"));
    }
